...
struct task_struct
{
	...
	struct task_struct *prev, *next;	/* Used to link the task struct in the
										   ...
										   runqueue */
/* ---------------- Do NOT Touch END-------------- */
	long double start_time;	//time the process started running
	long double last_burst;	//the amount of time that the process occupied the cpu the last time it ran
	long double next_expected_burst;	//the amount of time that we predicted the process will occupy the cpu the next time it will run
	long double last_time_runnable;	//the last time that the process was running
	long double waiting_in_rq;	//the amount of time that the process has been waiting in the runqueue
	long double goodness;	//indicates the goodness of the process
};
